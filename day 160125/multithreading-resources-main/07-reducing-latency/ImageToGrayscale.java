...
public class ImageToGrayscale {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            convertToGrayscaleConcurrently(image, 6);
    ...
    private static void convertToGrayscaleConcurrently(BufferedImage image, int numberOfThreads)
        ...
        GrayScaleCoverter[] thereads = new GrayScaleCoverter[numberOfThreads];
        for (int i = 0; i < thereads.length; i++) {
            int start = i * heightPerThread;
            int end = start + heightPerThread;

            thereads[i] = new GrayScaleCoverter(image, start, end);
            thereads[i].start();
        }

        for (GrayScaleCoverter thread : thereads) {
            thread.join();
        }
